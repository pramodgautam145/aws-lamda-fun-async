...
public class PatientCheckoutLamda {
    ...
    public void handler(S3Event event){
        event.getRecords().forEach(record->{
            ...
            try {
                ...
                patientCheckoutEvents.forEach(checkputEvent->{
                    try {
                        sns.publish(System.getenv("PATIENT_CHECKOUT_TOPIC"),objectMapper.writeValueAsString(checkputEvent));
